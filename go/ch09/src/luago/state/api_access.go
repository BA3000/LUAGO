...
// IsGoFunction 判断指定索引上的值是否可以转换成为Go函数；原理是直接获取索引上的值，然后直接看是否是闭包，并返回结果
func (lState *luaState) IsGoFunction(idx int) bool {
	val := lState.stack.get(idx)
...
// ToGoFunction 把指定索引处的值转换为Go函数并返回，如果无法转换则返回nil
func (lState *luaState) ToGoFunction(idx int) GoFunction {
	val := lState.stack.get(idx)
	if c, ok := val.(*closure); ok {
		return c.goFunc
	}
